        ...
        if repo_id is not None:
            ...
            if repo_name is not None:
                ...
                if name_match_mode == NameMatchMode.EXACT:
                    cmp = lambda x: x.name == repo_name if name_match_case else x.name.lower() == repo_name.lower()
                    repos_with_name = [x for x in repoyard_meta.repo_metas if cmp(x)]
                elif name_match_mode == NameMatchMode.CONTAINS:
                    cmp = lambda x: repo_name in x.name if name_match_case else repo_name.lower() in x.name.lower()
                    repos_with_name = [x for x in repoyard_meta.repo_metas if cmp(x)]
...
def _get_filtered_repo_metas(repo_metas, include_groups, exclude_groups, group_filter):
    if include_groups:
        repo_metas = [
            repo_meta for repo_meta in repo_metas if any(group in repo_meta.groups for group in include_groups)
        ]
    if exclude_groups:
        repo_metas = [
            repo_meta for repo_meta in repo_metas if not any(group in repo_meta.groups for group in exclude_groups)
        ]
    if group_filter:
...
def cli_rename(
    ...
    repo_id: str | None = Option(None, "--repo-id", "-i", help="The id of the repository to rename."),
    repo_name: str | None = Option(None, "--repo-name", "-n", help="The name of the repository to rename."),
    new_name: str = Option(..., "--new-name", "-N", help="The new name for the repository."),
...
def cli_sync_name(
    ...
    repo_id: str | None = Option(None, "--repo-id", "-i", help="The id of the repository."),
    repo_name: str | None = Option(None, "--repo-name", "-n", help="The name of the repository."),
...
def cli_copy(
    ...
    copy_meta: bool = Option(False, "--meta", help="Also copy repometa.toml."),
