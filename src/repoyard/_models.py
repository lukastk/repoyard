...
def generate_unique_repo_id(
    ...
    for _ in range(max_attempts):
        if config.repo_timestamp_format == RepoTimestampFormat.DATE_AND_TIME:
            creation_timestamp = datetime.now(UTC).strftime(const.REPO_TIMESTAMP_FORMAT)
        elif config.repo_timestamp_format == RepoTimestampFormat.DATE_ONLY:
            creation_timestamp = datetime.now(UTC).strftime(const.REPO_TIMESTAMP_FORMAT_DATE_ONLY)
        else:
...
class RepoMeta(const.StrictModel):
    ...
    def create(
        ...
        if creation_timestamp_utc is None:
            if config.repo_timestamp_format == RepoTimestampFormat.DATE_AND_TIME:
                creation_timestamp_utc = datetime.now(UTC).strftime(const.REPO_TIMESTAMP_FORMAT)
            elif config.repo_timestamp_format == RepoTimestampFormat.DATE_ONLY:
                creation_timestamp_utc = datetime.now(UTC).strftime(const.REPO_TIMESTAMP_FORMAT_DATE_ONLY)
            else:
                raise Exception(f"Invalid repo timestamp format: {config.repo_timestamp_format}")
        else:
    ...
    def get_remote_path(self, config: repoyard.config.Config) -> Path:
        return (
            config.storage_locations[self.storage_location].store_path / const.REMOTE_REPOS_REL_PATH / self.index_name
        )
...
class RepoyardMeta(const.StrictModel):
    ...
    def by_id(self) -> dict[str, RepoMeta]:
        if not hasattr(self, "__by_id"):
            self.__by_id = {repo_meta.repo_id: repo_meta for repo_meta in self.repo_metas}
        return self.__by_id
    ...
    def by_index_name(self) -> dict[str, RepoMeta]:
        if not hasattr(self, "__by_index_name"):
            self.__by_index_name = {repo_meta.index_name: repo_meta for repo_meta in self.repo_metas}
