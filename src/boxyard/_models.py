...
def generate_unique_box_id(
    ...
    for _ in range(max_attempts):
        if config.box_timestamp_format == BoxTimestampFormat.DATE_AND_TIME:
            creation_timestamp = datetime.now(timezone.utc).strftime(const.BOX_TIMESTAMP_FORMAT)
        elif config.box_timestamp_format == BoxTimestampFormat.DATE_ONLY:
            creation_timestamp = datetime.now(timezone.utc).strftime(const.BOX_TIMESTAMP_FORMAT_DATE_ONLY)
        else:
            raise Exception(f"Invalid box timestamp format: {config.box_timestamp_format}")

        box_subid = _create_box_subid(config.box_subid_character_set, config.box_subid_length)
...
class BoxMeta(const.StrictModel):
    ...
    def create(
        ...
        if creation_timestamp_utc is None:
            if config.box_timestamp_format == BoxTimestampFormat.DATE_AND_TIME:
                creation_timestamp_utc = datetime.now(timezone.utc).strftime(const.BOX_TIMESTAMP_FORMAT)
            elif config.box_timestamp_format == BoxTimestampFormat.DATE_ONLY:
                creation_timestamp_utc = datetime.now(timezone.utc).strftime(const.BOX_TIMESTAMP_FORMAT_DATE_ONLY)
            else:
                raise Exception(f"Invalid box timestamp format: {config.box_timestamp_format}")
        else:
            if "_" in creation_timestamp_utc:
                creation_timestamp_utc = creation_timestamp_utc.strftime(const.BOX_TIMESTAMP_FORMAT)
            else:
                creation_timestamp_utc = creation_timestamp_utc.strftime(const.BOX_TIMESTAMP_FORMAT_DATE_ONLY)
    ...
    def creation_timestamp_datetime(self) -> datetime:
        if "_" in self.creation_timestamp_utc:
            return datetime.strptime(self.creation_timestamp_utc, const.BOX_TIMESTAMP_FORMAT)
        else:
            return datetime.strptime(self.creation_timestamp_utc, const.BOX_TIMESTAMP_FORMAT_DATE_ONLY)
    ...
    def get_storage_location_config(self, config: boxyard.config.StorageConfig) -> boxyard.config.StorageConfig:
        return config.storage_locations[self.storage_location]

    def get_remote_path(self, config: boxyard.config.Config) -> Path:
        return (
            config.storage_locations[self.storage_location].store_path / const.REMOTE_BOXES_REL_PATH / self.index_name
        )
    ...
    def get_remote_part_path(self, config: boxyard.config.Config, box_part: BoxPart) -> Path:
    ...
    def get_local_part_path(self, config: boxyard.config.Config, box_part: BoxPart) -> Path:
        if box_part == BoxPart.DATA:
    ...
    def get_remote_sync_record_path(self, config: boxyard.config.Config, box_part: BoxPart) -> Path:
        sl_conf = self.get_storage_location_config(config)
        return sl_conf.store_path / const.SYNC_RECORDS_REL_PATH / self.index_name / f"{box_part.value}.rec"

    def get_local_sync_record_path(self, config: boxyard.config.Config, box_part: BoxPart) -> Path:
        return config.boxyard_data_path / const.SYNC_RECORDS_REL_PATH / self.index_name / f"{box_part.value}.rec"
    ...
    def load(
        ...
        boxmeta_path = config.local_store_path / storage_location_name / box_index_name / const.BOX_METAFILE_REL_PATH
    ...
    def validate_group_name(cls, group_name: str) -> None:
        ...
        if not isinstance(group_name, str) or not re.match(pattern, group_name):
            raise ValueError(f"Invalid group name '{group_name}'. Allowed characters: alphanumeric, '_', '-', '/'.")
...
class BoxyardMeta(const.StrictModel):
    ...
    def by_storage_location(self) -> dict[str, dict[str, BoxMeta]]:
        if not hasattr(self, "__by_storage_location"):
            ...
            self.__by_storage_location = {
                sl_name: {
                    box_meta.index_name: box_meta for box_meta in self.box_metas if box_meta.storage_location == sl_name
                }
    ...
    def by_id(self) -> dict[str, BoxMeta]:
        if not hasattr(self, "__by_id"):
            self.__by_id = {box_meta.box_id: box_meta for box_meta in self.box_metas}
        return self.__by_id
    ...
    def by_index_name(self) -> dict[str, BoxMeta]:
        if not hasattr(self, "__by_index_name"):
            self.__by_index_name = {box_meta.index_name: box_meta for box_meta in self.box_metas}
...
def create_boxyard_meta(config: boxyard.config.Config) -> BoxyardMeta:
    ...
    for storage_location_name in config.storage_locations:
        ...
        for box_path in local_storage_location_path.glob("*"):
            if box_path.is_file():
                continue
            box_metas.append(BoxMeta.load(config, storage_location_name, box_path.name))
...
def create_user_box_group_symlinks(
    ...
    box_metas = [box_meta for box_meta in get_boxyard_meta(config).box_metas if box_meta.check_included(config)]
        ...
        for p in path.iterdir():
            if p.is_dir():
                ...
                if p not in _symlink_paths:
                    raise Exception(f"File '{p}' is in the user box group path '{config.user_box_groups_path}'.")
    ...
    def _remove_empty_non_group_folders(path: Path) -> None:
        ...
        is_group_folder = path.relative_to(config.user_box_groups_path).as_posix() in groups
