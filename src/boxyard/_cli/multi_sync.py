...
def cli_multi_sync(
    box_index_names: list[str] | None = Option(None, "--box", "-r", help="The index names of the box, in the form."),
    ...
    sync_recently_modified_first: bool = Option(False, help="Sync boxes that have been recently modified first."),
    ...
    no_print_skipped: bool = Option(True, help="Do not print boxes for which no syncs happened."),
    ...
    if box_index_names is None:
        box_metas = [box_meta for box_meta in boxyard_meta.box_metas if box_meta.storage_location in storage_locations]
    else:
        if any(box_index_name not in boxyard_meta.by_index_name for box_index_name in box_index_names):
            typer.echo(f"Non-existent box: {box_index_names}")
            raise typer.Exit(code=1)
        box_metas = [boxyard_meta.by_index_name[box_index_name] for box_index_name in box_index_names]
    ...
    def get_status_lines(box_index_name):
        ...
        syncs_happened = [False if sync_results is None else sync_results[box_part][1] for box_part in sync_choices]
        lines.append(line)
