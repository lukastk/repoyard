...
def _run_with_lock_handling(coro):
    ...
    try:
        ...
        typer.echo(
            f"Hint: If you believe this is a stale lock, you can manually remove the lock file at: {e.lock_path}",
            err=True,
        )
...
def _get_box_index_name(
    ...
    if not allow_no_args and (box_name is None and box_index_name is None and box_id is None and box_metas is None):
    ...
    if sum(1 for x in [box_name, box_index_name, box_id] if x is not None) > 1:
        raise typer.Exit("Cannot provide more than one of `box-name`, `box-full-name` or `box-id`.")

    if name_match_mode is not None and box_name is None:
        raise typer.Exit("`box-name` must be provided if `name-match-mode` is provided.")

    if pick_first and box_name is None:
        raise typer.Exit("`box-name` must be provided if `pick-first` is provided.")

    search_mode = (box_id is None) and (box_name is None) and (box_index_name is None)
        ...
        if box_id is not None:
            ...
            if box_name is not None:
                ...
                if name_match_mode == NameMatchMode.EXACT:
                    cmp = lambda x: x.name == box_name if name_match_case else x.name.lower() == box_name.lower()
                    boxes_with_name = [x for x in boxyard_meta.box_metas if cmp(x)]
                elif name_match_mode == NameMatchMode.CONTAINS:
                    cmp = lambda x: box_name in x.name if name_match_case else box_name.lower() in x.name.lower()
                    boxes_with_name = [x for x in boxyard_meta.box_metas if cmp(x)]
            ...
            if len(boxes_with_name) == 0:
                ...
                if pick_first:
                    ...
                    _, box_index_name = run_fzf(
                        ...
                        disp_terms=[f"{r.name} ({r.box_id}) groups: {', '.join(r.groups)}" for r in boxes_with_name],
                    )
    ...
    if box_index_name is None:
        ...
        if box_index_name is None:
            raise typer.Exit("Box not specified and could not be inferred from current working directory.")
...
def cli_new(
    ...
    groups: list[str] | None = Option(None, "--group", "-g", help="The groups to add the new box to."),
    initialise_git: bool = Option(True, help="Initialise a git box in the new box."),
    ...
    if creation_timestamp_utc is not None:
        ...
        try:
            creation_timestamp_utc = datetime.strptime(creation_timestamp_utc, const.BOX_TIMESTAMP_FORMAT)
        except ValueError:
            try:
                creation_timestamp_utc = datetime.strptime(creation_timestamp_utc, const.BOX_TIMESTAMP_FORMAT_DATE_ONLY)
            except ValueError:
...
def cli_sync(
    box_path: Path | None = Option(None, "--box-path", "-p", help="The path to the box to sync."),
    box_index_name: str | None = Option(None, "--box", "-r", help="The index name of the box."),
    box_id: str | None = Option(None, "--box-id", "-i", help="The id of the box to sync."),
    box_name: str | None = Option(None, "--box-name", "-n", help="The name of the box to sync."),
...
def cli_add_to_group(
    ...
    group_name: str = Option(..., "--group", "-g", help="The name of the group to add the box to."),
...
def cli_yard_status(
    ...
    box_metas = [
        box_meta for box_meta in get_boxyard_meta(config).box_metas if box_meta.storage_location in storage_locations
    ]

    box_sync_statuses = asyncio.run(
        async_throttler(
            [get_formatted_box_status(config, box_meta.index_name) for box_meta in box_metas],
    ...
    for box_sync_status, box_meta in zip(box_sync_statuses, box_metas):
        box_sync_statuses_by_sl.setdefault(box_meta.storage_location, {})[box_meta.index_name] = box_sync_status
...
def _get_filtered_box_metas(box_metas, include_groups, exclude_groups, group_filter):
    if include_groups:
        box_metas = [box_meta for box_meta in box_metas if any(group in box_meta.groups for group in include_groups)]
    if exclude_groups:
        box_metas = [
            box_meta for box_meta in box_metas if not any(group in box_meta.groups for group in exclude_groups)
        ]
    if group_filter:
        from .._utils.logical_expressions import get_group_filter_func

        _filter_func = get_group_filter_func(group_filter)
        box_metas = [box_meta for box_meta in box_metas if _filter_func(box_meta.groups)]
    return box_metas
...
def cli_list_groups(
    ...
    box_index_name: str | None = Option(None, "--box", "-r", help="The box index name to get the groups of."),
    ...
    if list_all:
        group_configs, virtual_box_group_configs = get_box_group_configs(config, boxyard_meta.box_metas)
    ...
    if box_path is not None:
        ...
        if box_index_name is None:
            typer.echo("Could not determine the box index name from the provided box path.")
            raise typer.Exit(code=1)
    ...
    group_configs, virtual_box_group_configs = get_box_group_configs(config, [box_meta])
...
def cli_path(
    ...
    box_id: str | None = Option(None, "--box-id", "-i", help="The id of the box to sync."),
    box_name: str | None = Option(None, "--box-name", "-n", help="What box path to show."),
    ...
    only_included: bool = Option(True, "--only-included", "-o", help="Only show included boxes."),
    group_filter: str | None = Option(
        None,
        "--group-filter",
        "-f",
    ...
    if path_option == "data":
        ...
        typer.echo(box_meta.get_local_sync_record_path(config, BoxPart.DATA).as_posix())
    elif path_option == "sync-record-meta":
        typer.echo(box_meta.get_local_sync_record_path(config, BoxPart.META).as_posix())
    elif path_option == "sync-record-conf":
        typer.echo(box_meta.get_local_sync_record_path(config, BoxPart.CONF).as_posix())
    else:
...
def cli_rename(
    ...
    box_id: str | None = Option(None, "--box-id", "-i", help="The id of the box to rename."),
    box_name: str | None = Option(None, "--box-name", "-n", help="The name of the box to rename."),
    new_name: str = Option(..., "--new-name", "-N", help="The new name for the box."),
...
def cli_copy(
    ...
    dest_path: Path = Option(..., "--dest", "-d", help="Destination path for the copy."),
    copy_meta: bool = Option(False, "--meta", help="Also copy boxmeta.toml."),
    copy_conf: bool = Option(False, "--conf", help="Also copy conf/ folder."),
    overwrite: bool = Option(False, "--overwrite", help="Overwrite if dest exists."),
    show_rclone_progress: bool = Option(False, "--progress", help="Show the progress of the copy in rclone."),
):
    """
...
def cli_force_push(
    ...
    source_path: Path = Option(..., "--source", "-s", help="Source folder to push."),
    force: bool = Option(False, "--force", "-f", help="Required: confirm force overwrite."),
    show_rclone_progress: bool = Option(False, "--progress", help="Show the progress of the sync in rclone."),
    soft_interruption_enabled: bool = Option(True, help="Enable soft interruption."),
):
    """
...
def cli_which(
    path: Path | None = Option(
        ...
        "--path",
        "-p",
        help="The path to check. Defaults to current working directory.",
    ),
