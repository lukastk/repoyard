...
which is essentially equivalent to running `nbl export --pipeline "pts->nbs"`.
...
Files in the module directory (e.g., `my_package/`), except `__init__.py` and `__main__.py` files, are auto-generated. Any changes will be overwritten by `nbl export`.
...
nbl export --reverse # Reverse the pipeline (this only applies to notebook code locations, so not the module code locations)
